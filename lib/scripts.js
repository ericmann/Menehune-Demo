( function( window, $, undefined ) {
	...
	var workers = window.workerPool = new window.Workers( 'lib/worker.js' );
	workers.spawn( 3 );
...
window.console.log( iteration );
		...
		interval = window.setTimeout( doNext, 1 );
	}
	
	var monkeys = window.monkeys = {
		startTyping: function( text ) {
			// Clear out any existing data
			monkeys.updateProgress( '', 0, 0 );

			genetics = new window.Genetics( text, { useWorkers: $useWorkers.is( ':checked' ) } );
